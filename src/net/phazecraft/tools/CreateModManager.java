...
public class CreateModManager extends JDialog {
	...
	private class ModManagerWorker extends SwingWorker<Boolean, Void> {
		...
		protected boolean downloadModPack(String dir) throws IOException, NoSuchAlgorithmException {
			Logger.logInfo("Downloading Mods for the Pack");
			Logger.logInfo("Modpack: " + dir + "  Version: " + curVersion);
			TrackerUtils.sendPageView("net/ftb/tools/ModManager.java", "Downloaded: " + dir + " v." + curVersion.replace('_', '.'));
			...
			file.setText("Installing Mods");// Logger.logInfo(DownloadUtils.getCreeperhostLink("/"
											// + curVersion.replace(".", "_") +
											// "/" + modPackName.split(".")[0] +
											// ".txt"));
			...
			try (Scanner scanner = new Scanner(path)) {
				while (scanner.hasNextLine()) {
					...
					new File(installPath + "/" + dir + "/minecraft/mods/" + cLine).renameTo(new File(installPath + "/" + dir + "/minecraft/mods/" + cLine + ".disabled"));
				}
			}
			...
			for (int i = 0; i < new File(installPath + "/" + dir + "/minecraft/coremods").list().length; i++) {
				new File(new File(installPath + "/" + dir + "/minecraft/coremods").list()[i]).renameTo(new File(new File(installPath + "/" + dir + "/minecraft/coremods").list()[i] + ".disabled"));
			}
	 ...
	 * Create the frame.
	 */
	public CreateModManager(JFrame owner, Boolean model) {
		super(owner, model);
		setResizable(false);
